...
public class ProjectViewResource {
    ...
    public ResponseEntity<ProjectView> partialUpdateProjectView(
        ...
        Optional<ProjectView> result = projectViewRepository
            ...
            .map(existingProjectView -> {
                ...
                if (projectView.getImageName() != null) {
                    existingProjectView.setImageName(projectView.getImageName());
                }
                if (projectView.getImageData() != null) {
                    existingProjectView.setImageData(projectView.getImageData());
                }
                if (projectView.getImageDataContentType() != null) {
                    existingProjectView.setImageDataContentType(projectView.getImageDataContentType());
                }
    ...
    public ResponseEntity<List<ProjectView>> getAllProjectViews(
        @org.springdoc.api.annotations.ParameterObject Pageable pageable,
        @RequestParam(required = false, defaultValue = "true") boolean eagerload
    ) {
        log.debug("REST request to get a page of ProjectViews");
        Page<ProjectView> page;
        if (eagerload) {
            page = projectViewRepository.findAllWithEagerRelationships(pageable);
        } else {
            page = projectViewRepository.findAll(pageable);
        }
    ...
    public ResponseEntity<ProjectView> getProjectView(@PathVariable Long id) {
        ...
        Optional<ProjectView> projectView = projectViewRepository.findOneWithEagerRelationships(id);
