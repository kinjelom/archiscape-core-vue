...
/**
 * REST controller for managing {@link net.archiscape.app.core.domain.Project}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Project> result = projectRepository
            ...
            .map(existingProject -> {
                ...
                if (project.getConfiguration() != null) {
                    existingProject.setConfiguration(project.getConfiguration());
                }
                if (project.getExtProjectId() != null) {
                    existingProject.setExtProjectId(project.getExtProjectId());
                }
                if (project.getActive() != null) {
                    existingProject.setActive(project.getActive());
                }
                if (project.getSrcUrl() != null) {
                    existingProject.setSrcUrl(project.getSrcUrl());
                }
                if (project.getContentStoreUrl() != null) {
                    existingProject.setContentStoreUrl(project.getContentStoreUrl());
                }
