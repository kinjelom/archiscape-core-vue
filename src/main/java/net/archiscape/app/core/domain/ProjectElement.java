...
public class ProjectElement implements Serializable {
    ...
    @ManyToMany(mappedBy = "projectElements")
    @JsonIgnoreProperties(value = { "project", "projectElements" }, allowSetters = true)
    ...
    public void setProjectViews(Set<ProjectView> projectViews) {
        if (this.projectViews != null) {
            this.projectViews.forEach(i -> i.removeProjectElement(this));
        }
        if (projectViews != null) {
            projectViews.forEach(i -> i.addProjectElement(this));
        }
